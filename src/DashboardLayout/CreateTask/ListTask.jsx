...
const ListTask = ({ tasks, setTasks }) => {
  ...
  return (
    <div>
      <div className="flex gap-20 flex-col pb-7 md:flex-row">
...
const Header = ({ text, bg, count }) => {
  return (
    <div>
      <div
        className={` ${bg} flex items-center  h-10 uppercase rounded-md font-semibold text-white px-3`}
      >
        {text}{" "}
        <div className="ml-5 bg-red-500 w-5 h-5 text-white text-center font-semibold rounded-full flex items-center justify-center">
          {count}
        </div>{" "}
      </div>
    </div>
...
const Task = ({ task, tasks, setTasks }) => {
  ...
  return (
    <div
      ref={drag}
      className={`p-3 mt-8 shadow-md bg-slate-200 text-gray-500 space-y-4 rounded-md ${
    ...
    >
      ...
      <p className="text-lg font-bold">
        DeadLine : <span className="text-sm">{task.deadline}</span>
      </p>

      <div>
        <button
          onClick={() => handelRemove(task.title)}
          className="  mt-3 mr-3 bg-red-200 btn-sm rounded-lg drop-shadow-md "
        >
          <MdDelete className="text-2xl text-red-500 "></MdDelete>
        </button>
        <button
          onClick={() => document.getElementById("my_modal_5").showModal()}
          className=" bg-green-200 rounded-lg btn-sm  drop-shadow-md  "
        >
